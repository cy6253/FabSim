 ...
 *    누르면 그게 곧 선택이자 시점이다.
 *
 * 노드 에디터는 없앴다. 분기를 만드는 유일한 화면이었지만 실제로는 아무도
 * 안 썼고, 그 하나 때문에 @xyflow가 번들에 들어와 있었다. **실행 모델은 그대로
 * 분기를 지원한다** — 갈래가 든 프로젝트 파일은 여전히 열리고 돌아간다(가장 긴
 * 갈래를 고른다). 화면에서 새로 만들 수 없을 뿐이다.
 */
