 ...
 * 두 번 처음부터 다시 돌리고 있었는데 무거운 예제 한 벌이 8초다 — 시험이 느리면
...
describe("진단 — 문제를 잡아낸다", () => {
  ...
  it("실측 스텝 커버리지를 숫자로 보고한다 — 다만 컨포멀 선언에는 침묵한다", () => {
     ...
     * 형상에서는 그게 곧 스텝 커버리지지만, 구멍이나 슬릿을 **옆에서** 메우는
     * 성장은 그 컬럼에 안 잡힌다. allops의 금속 채움이 그렇다 — 선언 1.00짜리
     * LPCVD인데 컬럼으로 재면 0.67로 나온다. 막은 멀쩡한데 지표가 못 본 것이라,
     * 학생에게는 "컨포멀이라면서 왜 67%냐"로만 읽힌다.
     *
     * 그래서 컨포멀이라고 선언한 방법은 이 지표로 판정하지 않는다. 실제로
     * 잘못됐다면 봉인 보이드 진단이 잡는다 — 거기는 컬럼이 아니라 연결성으로
     * 세므로 옆으로 자란 몫도 제대로 본다.
     */
    const conformal = runWithDiagnostics("allops");
    expect(
      conformal.diags.filter((d) => d.kind === "coverage-measured"),
      "컨포멀만 쓰는 예제라 커버리지 진단이 하나도 없어야 한다",
    ).toEqual([]);
  });
