...
describe("CMOS 인버터 예제", () => {
  ...
  it("소자가 보이는 자리에서 열린다", () => {
     ...
     *
     * 자르는 것과는 다르다. 접는 것은 "이 재질은 지금 안 볼래"라 범례에 꺼진
     * 칩으로 남아 한 번 누르면 돌아오지만, 잘린 형상은 그냥 반쪽으로 보인다.
     */
    const v = p.view!;
    expect(v.hidden, "층간 절연막을 접고 연다").toContain("SiO2");
    // 자르지는 않는다 — 처음 보는 사람에게는 "왜 반만 있지"가 먼저 온다.
    // 다만 축은 y로 맞춰 둔다. 자르기로 마음먹었을 때 그쪽이 소자를 가른다.
    expect(v.cutX, "잘린 채로 열지 않는다").toBeUndefined();
    expect(v.cutAxis, "소자는 x를 따라 늘어서 있으니 자른다면 y다").toBe(1);
