...
export type ParamSpec =
  | {
       ...
       *
       * 길이와 시간은 격자와 무관한 절댓값일 수가 없다. 깊은 식각을 예로 들면,
       * 격자를 두 배로 촘촘히 한 순간 같은 깊이를 파는 데 두 배의 시간이 드는데
       * 상한이 그대로면 입력칸이 **조용히 잘라** 식각이 바닥에 못 닿는다.
       * 오류도 경고도 없이 결과만 달라지는 종류의 고장이다. 반대쪽도 있다:
       * "빠르게"(nz 60)에서는 두께 상한 60이 격자 전체다.
       *
 ...
 *
 * 상한을 스펙에 절댓값으로 박아 두면 격자를 바꿨을 때 뜻이 어긋난다. 깊은 식각은
 * 격자를 촘촘히 한 만큼 시간이 더 드는데, 상한이 그대로면 입력칸이 **조용히 잘라**
 * 식각이 바닥에 못 닿는다 — 오류도 경고도 없이 결과만 달라지는 종류의 고장이다.
 *
