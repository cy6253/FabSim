...
export interface Project {
   ...
   *
   * 예제도 이걸 쓸 수 있지만 **아끼는 편이 낫다.** 잘라 놓고 열어 봤더니 처음
   * 보는 사람에게는 "왜 반만 있지"가 먼저 왔고, 재질을 접어 놓고 열었더니
   * 그 재질이 나오는 단계마다 아무 일도 안 일어난 것처럼 보였다 — 시점은
   * **모든 단계에 걸리는데** 그 둘은 마지막 한 장만 보고 고른 값이었다.
   * 지금 남은 것은 단계와 무관하게 옳은 것들(절단 축, 완화)뿐이다.
   *
