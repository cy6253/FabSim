...
export function buildSmoothMesh(mat: Uint8Array, o: MeshOptions): Mesh {
   ...
   *
   * 재질별 점유도를 **삼선형으로** 재고 가장 큰 것을 고른다. 그러면 경계가 칸
   * 한가운데가 아니라 반 칸 자리에 놓여, 복셀 계단이 아니라 껍질과 같은 매끄러운
   * 곡선을 따라간다. 고르는 것은 여전히 하나뿐이라 경계 자체는 흐려지지 않는다.
   *
   * **흐리지는 않는다.** 예전에는 이 장들도 껍질과 같은 커널로 흐린 뒤 흐린
   * 값끼리 비교했는데, 그것은 이웃을 둘러본 다수결이라 두꺼운 재질이 구조적으로
   * 이긴다 — 한 복셀 막이 제 자리에서도 져서 화면에서 통째로 사라졌다. 원본을
   * 보도록 고치고 나니 흐린 값은 경계를 반 칸도 못 옮기는 덤이 됐다. 다섯 예제를
   * 완화 5로 렌더해 픽셀을 맞대 보니 차이가 0.00~0.29%였고, 그 값에 메시 시간
   * 두 배(500ms 대 200ms)와 격자마다 재질 수만큼의 4MB 배열을 치르고 있었다.
   */
  const labels: number[] = [];
  ...
  /** 재질별 점유도를 삼선형으로 재고 가장 큰 것을 고른다. 아무것도 없으면 -1. */
